...
이메일 : [dss99911@gmail.com](mailto:dss99911@gmail.com) | GitHub : [github.com/dss99911](https://github.com/dss99911) | LinkedIn : [linkedin.com/in/hyun-kim-7149b7b0](http://www.linkedin.com/in/hyun-kim-7149b7b0) | 블로그 : [jeonghyeon.kim](https://jeonghyeon.kim/)
...
| 기간 | 소속 | 역할 |
| :---- | :---- | :---- |
| 2026.01 ~ 현재 | (주)밸런스히어로 | AI/Data 팀 ML Part Leader |
| 2024.06 ~ 2025.12 | (주)밸런스히어로 | AI/Data 팀 Data Engineer |
| 2023.11 ~ 2024.05 | (주)밸런스히어로 | AI/Data 팀 ACS Part Leader |
| 2021.11 ~ 2023.10 | (주)밸런스히어로 | Data Science 팀 |
| 2015.03 ~ 2021.10 | (주)밸런스히어로 | 안드로이드 개발 팀 |
...
> 인도향 저소득층을 위한 소액대출 앱 서비스 (10M+ 다운로드)
> 현금 대출, EMI, 월렛, 커머스 등 제공 ([Google Play](https://play.google.com/store/apps/details?id=com.balancehero.truebalance&hl=en))

---

#### ML Part Leader (2026.01 ~ 현재)
**ML Workspace — AI 통합 개발 환경 구축** ([블로그](https://blog.afinit.com/perfect-ai-assistant-guide))
- 1 Server + 5 Clients(VSCode, Slack, Web, CLI, Scheduler) + 22 Skills 구조의 AI 개발 플랫폼을 설계 및 개발
...
**시스템 개선**
- Langchain, LangGraph를 사용하여 전사 Slack AI 챗봇 개발 — 사내 시스템과 연동된 AI 어시스턴트 ([블로그](https://blog.afinit.com/ai-agent-mcp-server-automation))
- ACS 호출 event-driven 구조로 전환. 성능, latency, 비용, 구조 단순화

---

#### ACS Part Leader (2023.11 ~ 2024.05)
ACS(대안 신용평가) 파트 리더로 피쳐/모델 개발 방향 결정 및 기술 전략 수립
...
- ACS 모니터링 체계 구축 — performance, consistency, business 모니터링 체계 구축

**ACS 실시간 서빙**
- Shadow testing 아키텍처 — 프로덕션 영향 없이 신규 모델을 검증할 수 있는 구조 구현
- ACS 실시간 서빙 응답시간 10초 → 5초 — AOP 스타일 함수 계층 분석 도구를 자체 개발하여 전체 함수의 병목 구간을 정밀 분석 후 최적화
- SageMaker endpoint 비용 5배 절감 — gunicorn+gevent 조합에서 process별 request 불균형 원인을 coroutine 내부 원리 분석으로 파악, gunicorn 제거로 해결
...
- NER 모델에 SMS 파싱 결과를 레이블로 활용하여 학습하고, NER ↔ 패턴 비교 파이프라인으로 잘못된 패턴을 자동 탐지·교정하는 피드백 루프 구축
- 신용보고서 기반 대출 상환기간 결측치 추정(특허 출원 40% 부분 기여)
...
**금융 SMS 해석 엔진**
- 인도 은행별 SMS 패턴을 분석하고 금융 거래 정보를 구조화하여 추출하는 엔진 전체 설계 및 개발 (안드로이드 + Spring 서버 + Spark)
...
- [피쳐별 모듈화](/mobile/android/2023/12/24/simple-architecture-refactoring-to-new-arhictecture.html), [테스트 방법론](/mobile/android/2023/12/24/testing-efficiently.html), [boilerplate 최소화](/mobile/android/2023/12/24/reduce-boiler-plate-code.html), API 에러/로딩 자동 처리 등 6편의 기술 블로그 시리즈 작성
