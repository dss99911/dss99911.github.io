...
## AI 에이전트를 서버에서 돌릴 때 다시 보는 Linux 보안

Claude Code, GitHub Copilot Workspace 같은 AI 에이전트를 서버에서 직접 쓰기 시작하면서, 평소 별 생각 없이 넘어가던 Linux 권한 모델을 다시 들여다보게 됐다. AI는 코드 작성, 파일 수정, 명령어 실행 등 **유저가 할 수 있는 거의 모든 것**을 자율적으로 한다.

문제는 같은 능력을 악성 스크립트도 갖는다는 점이다.
...
Linux 서버에서 다음 세 가지를 정리한다:
1. 기본 설정만으로 부족한 이유 (같은 UID = 같은 권한)
2. 시크릿을 격리해서 보관하는 방법
3. AI 에이전트에게 권한을 주면서도 폭주 위험을 줄이는 방법
...
결국 AI를 적극적으로 쓰면서도 안전을 유지하려면, 처음부터 보안을 고려해서 시스템을 설계해두는 수밖에 없다.
