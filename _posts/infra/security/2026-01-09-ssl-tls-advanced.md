...
HTTPS는 자물쇠 아이콘 뒤에 가려져 있어서 평소엔 의식하지 않다가, Charles Proxy로 패킷을 까보거나 자체 인증서로 사내 서비스를 띄울 때 갑자기 인증서 체인이라는 벽에 부딪힌다. 이 글에서는 인증서 체인이 어떻게 검증되는지, 자체 CA를 만들어 인증서를 발급하는 방법, 그리고 MITM 도구가 HTTPS를 들여다보는 원리를 차례로 정리한다.
