...
last_modified_at: 2026-05-30 18:30:00 +0900
...
### ⚠️ `firewall-cmd --reload` 후 재발 주의

위 PostUp 의 `iptables -I FORWARD ... ACCEPT` 두 줄은 **`firewall-cmd --reload` 시점에 함께 사라집니다**. 그 결과 핸드셰이크는 계속 성공하지만 트래픽만 막혀서, "VPN 연결됐는데 인터넷 안 됨" 증상이 갑자기 재현됩니다.

원인은 OL9/RHEL9 의 `iptables` 명령이 실제로는 `iptables-nft` (alternative alias) 라서, 내부적으로 nftables 의 `table ip filter` 를 조작하는데, `firewall-cmd --reload` 가 이 compat 테이블의 사용자 INSERT 룰을 함께 비워버리는 동작 때문입니다.

**규칙**: `firewall-cmd --reload` / `--add-port` / `--add-rich-rule` 등 firewalld 영구 설정을 만지면, **반드시 wg-quick 도 재시작**:

```bash
sudo firewall-cmd --permanent --add-port=51820/udp
sudo firewall-cmd --reload
sudo wg-quick down wg0 && sudo wg-quick up wg0   # ← 이걸 빠뜨리면 ACCEPT 룰 휘발
```
...
| 증상 | 확인 명령 | 정상값 |
|---|---|---|
| 핸드셰이크는 되나? | `sudo wg show` | `latest handshake: N seconds ago` |
| FORWARD 에 wg0 룰 있나? | `sudo iptables -L FORWARD -n --line-numbers \| head` | #1/#2 에 `ACCEPT ... wg0` |
| 패킷이 drop 되나? | `sudo iptables -L FORWARD -n -v \| head -1` | `policy DROP N packets` 가 누적 중이면 ❌ |
| wg0 트래픽 흐르나? | `sudo tcpdump -nni wg0` | 클라 사용 중일 때 패킷 흐름 |
