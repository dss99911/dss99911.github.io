...
서버 한두 대를 설치할 때는 USB나 ISO로 부팅해도 큰 문제가 없지만, 수십 대 이상의 베어메탈 서버를 손으로 만지는 건 현실적으로 불가능에 가깝다. PXE Boot는 이런 상황에서 네트워크만으로 부팅과 OS 설치를 자동화해주는 기술이다. 이 글은 PXE 환경을 구성할 때 알아둬야 할 핵심 요소를 정리한 메모다.
...
PXE Boot 환경을 한 번 구축해두면 베어메탈 서버를 다루는 일이 확연히 편해진다.

- USB/CD 없이 네트워크만으로 설치
- 수백 대 단위의 동시 설치 가능
- Kickstart/Preseed와 결합하면 무인 설치
- 모든 서버에 동일한 설정을 일관되게 적용

클라우드 환경에서는 AMI나 이미지 기반 프로비저닝으로 대체되지만, 온프레미스 데이터센터나 베어메탈 서버 환경에서는 PXE Boot가 여전히 현역이다.
