...
반복적으로 같은 DB 쿼리를 던지거나 같은 외부 API를 두드리고 있다는 걸 어느 순간 깨닫게 되는 시점이 있다. 이때 손에 가장 먼저 잡히는 게 Spring Cache다. 어노테이션 몇 개 붙이면 캐시 구현체와 상관없이 같은 코드로 동작하니, 일단 시도해 보기에 부담이 적다.
...
캐싱에서 가장 까다로운 부분은 결국 무효화 시점이다. 어노테이션 자체는 거의 그대로지만, 어떤 조건에서 캐시를 비울지 — 그리고 비웠을 때 다른 캐시와의 일관성은 어떻게 맞출지 — 까지 함께 설계해 두지 않으면 시간이 지난 뒤 묘한 데이터 불일치로 돌아온다. 로컬에서는 Caffeine, 운영에서는 Redis 정도가 무난한 출발선이다.
