...
"새벽에 한 번씩 돌아가야 하는 작업"이 한두 개 생기는 순간, cron 데몬을 따로 띄울지 코드 안에 넣을지 잠깐 고민하게 된다. Spring Boot에서는 `@Scheduled` 어노테이션 하나면 끝나기 때문에, 별다른 이유가 없으면 그냥 애플리케이션 안에서 같이 돌리는 쪽이 편하다.
...
`@Scheduled`는 작은 cron이 필요한 대부분의 상황에서 충분히 잘 동작한다. 단, 인스턴스가 둘 이상으로 늘어나는 순간 중복 실행 문제가 따라오니, 분산 환경에서는 ShedLock 같은 락 메커니즘을 함께 설계해 두는 게 마음 편하다.
