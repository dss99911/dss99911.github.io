...
컨트롤러 코드 안에 `try-catch`가 늘어나거나, 매 핸들러마다 같은 모델 속성을 채워 넣고 있는 자신을 발견했다면 `@ControllerAdvice`를 꺼낼 때가 된 것이다. 컨트롤러 전반에 걸쳐 공통으로 적용되는 로직을 한쪽에 모아둘 수 있는 자리다.
...
전역 예외 처리는 결국 "이 API는 어떤 에러 응답 모양을 가지는가"라는 약속을 코드 한곳에서 끝내는 작업이다. `@ControllerAdvice`로 한 번 잘 잡아두면 새 컨트롤러를 추가할 때마다 같은 고민을 반복할 필요가 없다. REST API라면 `@RestControllerAdvice`로 시작하는 게 거의 항상 정답에 가깝다.
